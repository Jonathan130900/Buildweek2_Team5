...
function printSong(data) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    showSong.innerHTML += ` <div class="tagStart d-flex p-0 rounded mt-3"> <img src="${albumCover}" class="rounded-start" style="width: 5em; height: 100%;" data-song="${song}" data-song-id="${data[i].id}"> <div> <h4 class="ms-2 text-light">${songTitle}</h4> <p class="ms-2 text-light">${artistName}</p> </div> </div>`;
  }
  ...
  songImages.forEach((img) => {
    img.addEventListener("click", function () {
      ...
      document.querySelector(
        "#playerSection .song-title-container h6"
      ).innerHTML = `${songTitle} - ${artistName}`;
...
function avviaBarraProgresso() {
  ...
  avviaBarraProgressoMobile();
}
function updateProgressBar() {
  progressBar.style.width = `${progress}%`;
  progressBarMobile.style.width = `${progress}%`;
  updateCurrentTime(currentAudio.currentTime);
}
...
function avviaBarraProgressoMobile() {
  mobileProgressInterval = setInterval(() => {
    if (currentAudio && !currentAudio.paused) {
      progress += (100 / currentAudio.duration) * 0.1;
      if (progress >= 100) {
